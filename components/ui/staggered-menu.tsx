...
export interface StaggeredMenuLanguageItem {
  code: string;
  label: string;
  flag: string;
}
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            ref={toggleBtnRef}
            className="sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto text-white text-[1.5em]"
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <Link
                      className="sm-panel-item relative text-black font-semibold text-[2.5rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em] pointer-events-auto"
                      href={it.link}
                    ...
                    >
                      <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform pointer-events-none">
                        {it.label}
                      </span>
                    </Link>
                  </li>
            ...
            {languageItems && languageItems.length > 0 && (
              <div className="sm-languages mt-auto pt-6 flex flex-col gap-2" aria-label="Language selector">
                <h3 className="sm-languages-title m-0 text-base font-medium [color:var(--sm-accent,#ff0000)]">{languageLabel}</h3>
                <div className="sm-languages-list flex flex-row items-center gap-3 flex-wrap">
                  {languageItems.map((lang) => (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        // Cerrar el menú después de cambiar idioma
                        setTimeout(() => {
                          toggleMenu();
                        }, 300);
                      }}
                      className={`sm-language-btn flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer ${
                        currentLanguage === lang.code
                          ? 'bg-[var(--sm-accent,#2563eb)] text-white'
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div className="sm-socials pt-4 flex flex-col gap-2" aria-label="Social links">
...
.sm-scope .sm-toggle { position: relative; display: inline-flex; align-items: center; gap: 0.3rem; background: transparent; border: none; cursor: pointer; color: white; font-weight: 500; line-height: 1; overflow: visible; font-size: 1.5em; }
...
.sm-scope .sm-socials { margin-top: 0; padding-top: 0.5rem; display: flex; flex-direction: column; gap: 0.75rem; }
